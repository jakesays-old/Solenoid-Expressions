...
namespace Solenoid.Expressions.Tests.Util
{
	...
	public sealed class AssertUtilsTests
	{
		...
		public void UnderstandsType()
		{
			...
			AssertNotUnderstandsType(new object(), "target", null, typeof (ArgumentNullException),
				"Argument 'requiredType' cannot be null.");
		}
		...
		private void AssertNotUnderstandsType(object target, string targetName, Type requiredType, Type exceptionType,
		...
		private void AssertNotUnderstandsMethod(object target, string targetName, MethodBase method, Type exceptionType,
			string partialMessage)
		{
			try
			{
		...
		private class TestProxy : RealProxy, IRemotingTypeInfo
		{
			...
			{
				var res = fromType.IsAssignableFrom(targetInstance.GetType());
				return res;
			}
