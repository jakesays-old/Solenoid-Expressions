...
namespace Solenoid.Expressions.Support.Util
{
	...
	public static class ReflectionUtils
	{
		...
		/// </summary>
		private const BindingFlags AllMembersCaseInsensitiveFlags = BindingFlags.Public |
			...
			Type type, string method, Type[] argumentTypes)
		{
			...
			{
				sb.Append(separator).Append(t.FullName);
				separator = ",";
			}
			sb.Append(")");
			return sb.ToString();
			...
			Type targetType, string method, Type[] argumentTypes, int genericArgumentsCount)
		{
			...
			foreach (var candidate in methods)
			{
				...
				{
					var parameterTypes = Array.ConvertAll<ParameterInfo, Type>(candidate.GetParameters(),
		...
		/// <summary>
		///     Resolves a given <paramref name="methodInfo" /> to the <see cref="MethodInfo" /> representing the actual
		///     implementation.
		/// </summary>
		/// <remarks>
		///     see article
		///     <a href="http://weblog.ikvm.net/CommentView.aspx?guid=7356a87f-e5d7-4723-ae49-b263ab9e40ae">
		///         How To Get an Explicit
		...
		/// <returns>
		///     the <see cref="MethodInfo" /> representing the actual implementation method of the specified
		...
		public static MethodInfo MapInterfaceMethodToImplementationIfNecessary(MethodInfo methodInfo, Type implementingType)
		{
			...
			AssertUtils.IsTrue(methodInfo.DeclaringType.IsAssignableFrom(implementingType),
				"methodInfo and implementingType are unrelated");
		...
		public static MethodInfo GetGenericMethod(Type type, string methodName, Type[] typeArguments, Type[] parameterTypes)
		{
			...
			{
				// Non-Generic Method
				methodInfo = type.GetMethod(methodName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, null,
					parameterTypes, null);
			}
			else
			{
		...
		/// <summary>
		///     From a given list of methods, selects the method having an exact match on the given <paramref name="argValues" />'
		///     types.
		/// </summary>
		...
		///     If more than 1 matching methods are found in the <paramref name="methods" /> list.
		/// </exception>
		private static MethodBase GetMethodBaseByArgumentValues<T>(string methodTypeName, IEnumerable<T> methods,
			object[] argValues) where T : MethodBase
		{
			...
			foreach (var m in methods)
			{
				...
				var paramValues = argValues ?? new object[0];
				...
				try
				{
					if (parameters.Length > 0)
					{
						...
						if (lastParameter.GetCustomAttributes(typeof (ParamArrayAttribute), false).Length > 0 &&
							argValues.Length >= parameters.Length)
						{
							paramValues =
		...
		/// <summary>
		///     From a given list of constructors, selects the constructor having an exact match on the given
		///     <paramref name="argValues" />' types.
		/// </summary>
		...
		///     If more than 1 matching methods are found in the <paramref name="methods" /> list.
		/// </exception>
		public static ConstructorInfo GetConstructorByArgumentValues<T>(IList<T> methods, object[] argValues)
			where T : MethodBase
		{
		...
		public static bool PropertyIsIndexer(string propertyName, Type type)
		{
			...
			if (attribs.Length != 0)
			{
				return attribs.Any(attrib => attrib.MemberName.Equals(propertyName));
			}
			return false;
		}
			...
			MethodInfo candidate, Type[] parameterTypes)
		{
			...
			{
				if (!(candidatesParameterTypes[i] == parameterTypes[i]))
				{
					return false;
				}
			}
	...
	//TODO: investigate whether there is another equivalent manner of providing this functionality under MONO
		...
		/// <summary>
		///     Within <paramref name="type" />, counts the number of overloads for the method with the given (case-insensitive!)
		///     <paramref name="name" />
		/// </summary>
		...
		public static IList GetCustomAttributes(MemberInfo member)
		{
			...
			try
			{
				...
				else if (attrsData.Count > 0)
				{
					...
					{
						var hasSecurityAttribute = attrsData.Any(
							cad => typeof (SecurityAttribute)
								.IsAssignableFrom(cad.Constructor.DeclaringType));
		...
		public static bool IsTypeVisible(Type type, string friendlyAssemblyName)
		{
			...
			if (!string.IsNullOrEmpty(friendlyAssemblyName)
					...
					(!type.IsNestedPrivate && (type.IsNestedAssembly || type.IsNestedFamORAssem))))
			{
				...
				var results =
					attrs.Cast<InternalsVisibleToAttribute>().Any(ivta => ivta.AssemblyName.Split(',')[0] == friendlyAssemblyName);
				return results;
			}
			return false;
		}
		...
		public static Type[] GetInterfaces(Type type)
		{
			...
			if (type.IsInterface)
			{
				var interfaces = new List<Type> {type};
		...
		///     <list type="bullet">
		///         <item>
		///             <description>fromObject.GetType() == toObject.GetType()</description>
		///         </item>
		///         <item>
		///             <description>fromObject.GetType() is derived from toObject.GetType()</description>
		///         </item>
		///         <item>
		///             <description>toObject.GetType() is derived from fromObject.GetType()</description>
		///         </item>
		///     </list>
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		///     Useful in certain instances during testing to avoid the need to add protected properties, etc. to a class just
		...
		public static object GetInstanceFieldValue(object obj, string fieldName)
		{
			...
			var field = obj.GetType()
				.GetField(fieldName, BindingFlags.SetField | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
			if (field != null)
			{
				return field.GetValue(obj);
			}
			throw new ArgumentException(string.Format(
				"Non-public instance field '{0}' could not be found in class of type '{1}'", fieldName, obj.GetType().ToString()));
		}
		...
		public static void SetInstanceFieldValue(object obj, string fieldName, object fieldValue)
		{
			...
			if (field != null)
			{
				...
				{
					throw new ArgumentException(
						string.Format("fieldValue for fieldName '{0}' of object type '{1}' must be of type '{2}' but was of type '{3}'",
							fieldName, obj.GetType().ToString(), field.FieldType.ToString(), fieldValue.GetType().ToString()), "fieldValue");
				}
			...
			{
				throw new ArgumentException(
					string.Format("Non-public instance field '{0}' could not be found in class of type '{1}'", fieldName,
		...
		private static readonly Dictionary<Type, MemberwiseCopyHandler> _handlerCache =
			new Dictionary<Type, MemberwiseCopyHandler>();
		...
		private static MemberwiseCopyHandler GetImpl(Type type)
		{
			...
			lock (_handlerCache)
			{
				...
				{
					var dm = new DynamicMethod(type.FullName + ".ShallowCopy", null, new Type[] {typeof (object), typeof (object)},
						type.Module, true);
