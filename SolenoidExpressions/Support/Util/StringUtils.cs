...
namespace Solenoid.Expressions.Support.Util
{
	...
	public static class StringUtils
	{
		...
		public static string[] Split(
			string target, string delimiters, bool trimTokens, bool ignoreEmptyTokens)
		{
			return Split(target, delimiters, trimTokens, ignoreEmptyTokens, null);
		}
		...
		public static string CollectionToDelimitedString<T>(
			IEnumerable<T> collection, string delimiter)
		{
			if (collection == null)
			{
				return "null";
		...
		public static IList<string> GetAntExpressions(string text)
		{
			...
			if (HasText(text))
			{
				var start = text.IndexOf(AntExpressionPrefix, StringComparison.Ordinal);
				while (start >= 0)
				{
					...
					else
					{
						...
						start = text.IndexOf(AntExpressionPrefix, end, StringComparison.Ordinal);
					}
				}
			}
			return expressions;
		...
		///     <see cref="""" /> value if the supplied
		...
		///         <see langword="null" />, the <see cref="System.Empty" /> string will be returned.
		///     </p>
		/// </remarks>
		/// <param name="fix">
