...
{
	/// <summary>
	///     <see cref="DictionarySet" /> is an
	///     <see langword="abstract" /> class that supports the creation of new
	///     <see cref="ISet" /> types where the underlying data
	...
	public abstract class DictionarySet : Set
	{
		...
		private static readonly object _nullPlaceHolderKey = new object();

		/// <summary>
		///     Provides the storage for elements in the
		///     <see cref="ISet" />, stored as the key-set
		...
		{
			var result = collection.Cast<object>().Aggregate(false, (current, o) => current | Add(o));
			return result;
		}
		...
		public override bool ContainsAll(ICollection collection)
		{
			...
			var result = collection.Cast<object>().All(o => Contains(MaskNull(o)));
			return result;
		}
		...
		{
			var result = collection.Cast<object>().Aggregate(false, (current, o) => current | Remove(o));
			return result;
		}
		...
		/// <summary>
		///     Copies the elements in the <see cref="ISet" /> to
		///     an array.
		/// </summary>
		/// <remarks>
		///     <p>
		///         The type of array needs to be compatible with the objects in the
		///         <see cref="ISet" />, obviously.
		///     </p>
		/// </remarks>
		/// <param name="array">
		...
		///     <see cref="ISet" /> is synchronized across
		///     threads.
		/// </summary>
		/// <seealso cref="Set.IsSynchronized" />
		...
		{
			return key ?? _nullPlaceHolderKey;
		}

		private static object UnmaskNull(object key)
		{
			return key == _nullPlaceHolderKey ? null : key;
		}
