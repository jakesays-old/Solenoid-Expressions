...
namespace Solenoid.Expressions.Support.Logging
{
	...
	/// <remarks>
	///     <para>
	///         Implementations should defer calling a message's <see cref="object.ToString()" /> until the message really
	///         needs
	///         to be logged to avoid performance penalties.
	///     </para>
	///     <para>
	///         Each <see cref="ILog" /> log method offers to pass in a
	///         <see
	///             cref="System.Action{FormatMessageHandler}" />
	///         instead of the actual message.
	///         Using this style has the advantage to defer possibly expensive message argument evaluation and formatting (and
	///         formatting arguments!) until the message gets
	...
	///     <see cref="Random.NextDouble" /> and the underlying <see cref="string.Format(string,object[])" /> only happens, if
	///     level <see cref="LogLevel.Debug" /> is enabled:
	///     <code>
